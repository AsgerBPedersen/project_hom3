...
export const transports = [
  { label: "offentlig transport", value: 1 },
  { label: "cykle", value: 2 },
  { label: "gå", value: 3 },
  { label: "bil", value: 4 }
];
...
class Map extends Component {
  state = {
    ...
    transportSelect : 1,
    //no need to put map and polygon in state.
    map: null,
    polygon: null,
    stepper: true
  };
  ...
  sendTimeMapRequest = async () => {
    const cords = await this.getCords(this.state.inputs.adresse.value);
    console.log(cords);
  ...
  componentDidMount() {
    this.state.map = L.map("mapid").setView(
      [this.props.cords.lat, this.props.cords.lon],
      13
    );
    L.tileLayer(
      "https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token={accessToken}",
      {
        ...
        id: "mapbox.streets",
        accessToken:
          "pk.eyJ1IjoiaW56b2xpbjIiLCJhIjoiY2szaDM0bHQ4MDYxcjNkcGhpcTNrNnNlMCJ9.y8RASuL3s-eLjX-TybEzzQ"
      }
    ...
    fetch("http://localhost:4466", {
      ...
      body: JSON.stringify({
        query: "{ homes { lat lon title rooms size address price }}"
      })
    })
      .then(res => res.json())
      .then(res => {
        res.data.homes.forEach(
          ({ lat, lon, title, address, price, size, rooms }) => {
            if (lat != null) {
              markers.addLayer(
    ...
    return (
      <div>
        ...
        <Inputfield
          ...
          transportSelect={this.state.transportSelect}
          onClick={this.sendTimeMapRequest}
        ></Inputfield>
        {/* <Steppers
          inputs={this.state.inputs}
          onChange={this.onChange}
